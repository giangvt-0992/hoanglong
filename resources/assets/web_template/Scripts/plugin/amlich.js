...
function getSelectedMonth2(m,y) {
    var query = window.location.search;
    var arr = parseQuery(query);
    var idx;
    for (idx = 0; idx < arr.length; idx++) {
...
function printStyle() {
	...
	res += '<!--\n';
	//res += '  body {margin:0}\n';
	res += '  .tennam {text-align:center; font-size:150%; line-height:120%; font-weight:bold; color:#000000; }\n';
	res += '  .thang {font-size: '+fontSize+'; padding:1; line-height:100%; font-family:Tahoma,Verdana,Arial; table-layout:fixed}\n';
	res += '  .tenthang {text-align:center; font-size:125%; line-height:100%; font-weight:bold; color:#330033; }\n';
	res += '  .navi-l {text-align:center; font-size:75%; line-height:100%; font-family:Verdana,Times New Roman,Arial; font-weight:bold; color:red; }\n';
	res += '  .navi-r {text-align:center; font-size:75%; line-height:100%; font-family:Verdana,Arial,Times New Roman; font-weight:bold; color:#330033; }\n';
	...
	res += '  .sweet-alert {width:546px;}\n';
	res += '  .sweet-alert h2 { word-break: break-word;}\n';
	res += '-->\n';
	res += '</style>\n';
	return res;
}
...
function printTable(mm, yy) {
	...
	var res = "";
	res += ('<table class="table table-bordered table-striped thang" border="2" cellpadding="1" cellspacing="1" width="'+PRINT_OPTS.tableWidth+'">\n');
	res += printHead(mm, yy);
	for (i = 0; i < 6; i++) {
		res += ("<tr>\n");
		for (j = 0; j < 7; j++) {
...
function getPrevMonthLink(mm, yy) {
	...
	//return '<a href="javascript:;" onclick="getSelectedMonth2(' +mm1+',' +yy1+')">&lt;</a>';
}
...
function printFoot() {
	var res = "";
	//res += '<script language="JavaScript" src="~/scripts/plugin/amlich.js"></script>\n';
	return res;
}
