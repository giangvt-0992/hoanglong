							...
							<form class="form-horizontal form-label-left" method="POST" action="{{route('admin.trip.store')}}">
								...
								<option value=""></option>
									@foreach ($routes as $route)
											...
											@if($route->id == old('routeId')) selected @endif
								...
								<input type="text" class="form-control" placeholder="Tên chuyến" name="name" value="{{old('name')}}" required>
									@if($errors->has('name'))
								...
								<div class="form-group col-md-6 pl-0">
									<label>Giờ đi</label>
									<input type="text" class="form-control timepicker" name="departTime" id="departTime" placeholder="Giờ đi" value="{{old('departTime')}}" readonly required>
									@if($errors->has('name'))
								...
								<div class="form-group col-md-6 pr-0">
									<label>Giờ đến</label>
									<input type="text" class="form-control timepicker" name="arriveTime" id="arriveTime" placeholder="Giờ đến" value="{{old('arriveTime')}}" readonly required>
								</div>
								<div class="clear"></div>
								<div id="scheduleWrapper">
									<label>Lịch đón khách dự kiến</label>
									<?php 
									$scheduleDepartTime = old('scheduleDepartTime', []);
									$placeId = old('placeId', []);
									$countSchedule = count($scheduleDepartTime);
									?>
									@if ( $countSchedule > 0)
										@for ($i = 0; $i < $countSchedule; $i++)
										<div class="form-group schedule">
											<div class="col-md-4 pl-0">
												<input type="text" class="form-control timepicker" name="schedule[{{$i}}][time]" id="scheduleDepartTime{{$i}}" value="{{$scheduleDepartTime[$i]}}" placeholder="Thời gian" readonly required>
											</div>
											<div class="col-md-7 pr-0">
												<select class="form-control select2 list-place" name="schedule[{{$i}}][place_id]" id="slGetOffPlaceId{{$i+1}}" required>
													<option value=""></option>
													@foreach ($oldRoutePlaces as $place)
														<option
														value="{{$place->id}}"
														@if($place->id == $placeId[$i]) selected @endif
														>{{$place->name}}</option>
													@endforeach
												</select>
											</div>
									...
									<div class="form-group schedule">
										...
										<div class="col-md-7 pr-0">
											<select class="form-control select2 list-place" name="schedule[0][place_id]" id="slGetOffPlaceId1" required>
											</select>
										</div>
										<div class="col-md-1">
											<button class="btn btn-success" id="btnAddSchedule"><i class="fa fa-plus"></i></button>
								...
								<div class="form-group">
									...
									<select class="form-control" name="carTypeId" id="" required>
										@foreach ($carTypes as $carType)
												<option
												value="{{$carType->id}}"
												@if($carType->id == old('carTypeId')) selected @endif
												>{{$carType->name}}</option>
										@endforeach
									</select>
								</div>
...
<script>
	...
	// $("#scheduleDepartTime1").timepicker({
	//   format: 'HH:MM'
	// });
